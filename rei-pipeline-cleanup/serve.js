 ...
 * Open http://localhost:8787, set options, click Start.
 */
...
.app{max-width:1240px;margin:0 auto;padding:18px 26px 60px}
.apphead{display:flex;align-items:center;gap:14px;padding:10px 2px 16px;border-bottom:1px solid var(--line);margin-bottom:18px}
...
.srcbar{display:flex;align-items:center;gap:10px;flex-wrap:wrap;color:var(--ink-2);font-size:13px;margin-bottom:12px}
.srcbar b{color:var(--ink)}.srcbar a{color:var(--accent);text-decoration:none}.srcbar a:hover{text-decoration:underline}
details.info{background:var(--panel);border:1px solid var(--line);border-left:3px solid var(--accent);border-radius:12px;padding:0 16px;margin-bottom:18px}
...
details.info>summary::before{content:"▸ ";color:var(--muted)}
details.info[open]>summary::before{content:"▾ "}
details.info .body{padding:2px 0 14px;color:var(--ink-2);font-size:13.5px}
details.info .body b{color:#fff}
...
.firstrun{background:var(--panel);border:1px dashed var(--line-2);border-radius:12px;padding:26px;text-align:center;color:var(--muted);margin-bottom:14px}
...
.tbar{display:flex;align-items:center;gap:14px;flex-wrap:wrap;background:var(--panel);border:1px solid var(--line);border-radius:12px;padding:12px 14px;margin-bottom:14px}
...
.seg button{background:var(--panel-2);color:var(--ink-2);border:0;padding:8px 14px;font-weight:700;font-size:13px;cursor:pointer}
.seg button.on{background:var(--accent);color:#fff}.seg button.on.liveon{background:var(--green-2)}
select{background:var(--panel-2);color:var(--ink);border:1px solid var(--line-2);border-radius:9px;padding:8px 10px;font-size:13px}
...
.modal h2{margin:0;font-size:20px}.modal .muted{color:#6d7a92;font-size:13px}
...
.modal .krow .k{font-size:12px;color:#6d7a92;text-transform:uppercase;letter-spacing:.04em}.modal .krow .kv{font-size:24px;font-weight:750}
.modal h4{font-size:13px;text-transform:uppercase;letter-spacing:.05em;color:#6d7a92;margin:18px 0 8px}
.modal table{width:100%;border-collapse:collapse;font-size:13px}.modal th,.modal td{text-align:left;padding:8px 8px;border-bottom:1px solid #eef2f8}.modal th{color:#6d7a92;font-size:11px;text-transform:uppercase}
.modal .btn{background:#2f6bff;color:#fff;border:0;border-radius:8px;padding:8px 14px;font-weight:700;cursor:pointer;margin-left:8px}.modal .btn.ghost{background:#eef2f8;color:#141c2e}
...
.hide{display:none}
  ...
  <div class="srcbar"><b>Source:</b> New bucket in REI BlackBook
    ...
    · a Chrome window opens on Start — log into REI there once.</div>

  <details class="info"><summary>How it decides — from the rep's Lead Stage (no guessing)</summary>
    <div class="body"><b>Follow up</b> — worked lead (call/text/offer) → set Evaluating. &nbsp;<b>Dead</b> — Lost / Invalid / Wrong number → set Dead.<br/>
    <b>Under Contract</b>, conflicts &amp; review → held for a human and an explanatory note is written automatically. &nbsp;<b>New</b> — no contact / not worked → left as-is. Every change is reversible; the State field is never touched.</div>
  </details>

  <p class="hinttxt">▸ Click any card below to see those leads with links into REI.</p>
  <div class="stats" id="stats"></div>
  <div class="firstrun hide" id="firstrun"><b>No run yet.</b> Choose Audit or Live below and click Start — results appear here.</div>
...
async function startRun(resume){
  if(live && !resume && !confirm('LIVE mode will WRITE statuses + notes in REI (Follow up / Dead / notes). Continue?'))return;
...
async function openReport(){
  ...
  const explain='Today the automation reviewed <b>'+c.total+'</b> leads from the New pipeline. It set <b>'+c.ev+'</b> to <b>Follow up</b> and <b>'+c.cl+'</b> to <b>Dead</b> (both from the rep\\'s own Lead Stage), held <b>'+c.uc+'</b> Under-Contract lead(s) for a person, and flagged <b>'+c.mr+'</b> for review'+(c.dup?(' plus <b>'+c.dup+'</b> duplicate(s)'):'')+'. The remaining <b>'+c.new+'</b> had no contact/activity and were left as New. Every change is reversible; the State field was never touched.';
